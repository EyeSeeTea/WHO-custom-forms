...
export class D2AntivenomEntriesRepository implements AntivenomEntriesRepository {
    ...
    async get(): Promise<AntivenomEntries> {
        const antivenomEntries = await this.dataStoreClient.get<AntivenomEntries>(
            antivenomEntriesKey
        );
        ...
        if (!antivenomEntries) {
            throw new Error(
                `Does not exist a required ${this.dataStoreClient.namespace} namespace with a ${antivenomEntriesKey} key in the data store`
            );
        }
